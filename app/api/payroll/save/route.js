...
export async function POST(request) {
  ...
  if (!pay_month || !Array.isArray(rows) || rows.length === 0) {
    return NextResponse.json({ error: "A month and at least one staff row are required." }, { status: 400 });
  }
  ...
  if (records.length === 0) return NextResponse.json({ error: "No amounts entered." }, { status: 400 });

  // Upsert: if a (staff_id, pay_month) row exists, overwrite it; else insert.
  const { error } = await supabase
    .from("salaries")
    .upsert(records, { onConflict: "staff_id,pay_month" });
